...
struct BirdCargoConfig {
    static let appID = "6762218996"
    static let devKey = "mGXyDXz7o2He2kCTMhoyAa"
}
...
final class AttributionBridge: NSObject {
    ...
    func receiveNavigation(_ data: [AnyHashable: Any]) {
        guard !UserDefaults.standard.bool(forKey: "bcf_first_launch_flag") else { return }
        navigationBuf = data
        onNavigation?(data)
        timer?.invalidate()
...
final class AppDelegate: UIResponder, UIApplicationDelegate, MessagingDelegate, UNUserNotificationCenterDelegate {
    ...
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String?) {
        messaging.token { token, error in
            ...
            UserDefaults(suiteName: "group.birdcargo.storage")?.set(token, forKey: "shared_fcm")
        }
    }
...
final class SDKBridge: NSObject, AppsFlyerLibDelegate, DeepLinkDelegate {
    ...
    func configure() {
        ...
        sdk.appsFlyerDevKey = BirdCargoConfig.devKey
        sdk.appleAppID = BirdCargoConfig.appID
        sdk.delegate = self
